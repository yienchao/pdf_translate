...
WHAT IT DOES:
    1. Processes all PDFs in original/ folder
    2. Uses dictionary (4,330+ entries) to translate French → English
    3. Redacts original French text (properly removes it)
    4. Inserts English translations in same position
    5. Saves to translated_pdfs/ folder

KEY IMPROVEMENTS vs Method 9:
    ✓ Uses redaction (not white rectangles) - actually removes French text
    ✓ 95% English accuracy (vs Method 9's ~20%)
    ✓ Dual dictionary: base (92 words) + growing (4,330+ phrases)
    ✓ Better text merging (tighter threshold = fewer errors)

TRANSLATION WORKFLOW:
    - If text in dictionary → translate automatically
    - If French detected but not in dictionary → save to method12_data/{PDF}_to_translate.json
    - Use Claude Code agent to translate the JSON file
    - Save as method12_data/{PDF}_sentences.json
    - Re-run script to apply

FOLDERS:
    original/          → Input: French PDFs
    translated_pdfs/   → Output: English PDFs
    method12_data/     → Dictionaries and translation files

ADJUSTABLE PARAMETERS:
    Line 125: close_horizontal threshold (5px) - controls text merging
    Line 283: Redaction padding (0.5px) - controls white gap size

See METHOD12_README.md for full documentation.
"""
import fitz
import json
import os
...
def translate_with_dict(text):
    ...
    # No word-by-word fallback - only exact phrase matches
    # This prevents Franglais (mixed French/English)
    return None
...
def process_pdf(input_path, output_path):
    ...
    if needs_translation:
        ...
        for idx, elem in enumerate(text_elements):
            if elem.get("type") == "needs_translation":
                ...
                # Validate: reject identity mappings (French == English)
                if original != translated:
                    new_dict_entries[original] = translated
                else:
                    skipped += 1
                    print(f"  [SKIPPED] Identity mapping: {original[:50]}")

        # Update dictionary
        if new_dict_entries:
            print(f"Adding {len(new_dict_entries)} entries to dictionary")
            if skipped > 0:
                print(f"Skipped {skipped} identity mappings")
    ...
    for page_num in range(len(doc)):
        ...
        # Remove original text by redacting it (transparent, no white boxes)
        for elem in page_elements:
            bbox = elem["bbox"]
            # Use tight bbox with minimal padding
