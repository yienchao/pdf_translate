"""
Method 9: Text-Keyed Translation
- Short text (1-6 words): Dictionary lookup
- Sentences (7+ words): Text-as-key cache, batch translate with agent
- No numeric indexes = no corruption when PDFs evolve!
"""
import fitz
import json
import re
...
def translate_pdf_indexed(input_path, output_path):
    """Method 9: Text-keyed sentence translation"""
    print("="*80)
    print("METHOD 9: TEXT-KEYED TRANSLATION")
    print("="*80)
    print(f"Opening PDF: {input_path}")

    # Get PDF code for per-PDF sentence cache (e.g., "A-001", "A-530")
    pdf_code = os.path.basename(input_path).split(' - ')[0]
    sentences_cache = f"method9_data/{pdf_code}_sentences.json"
    ...
    sentences_to_translate = {}  # {text: placeholder for translation}
    ...
    for page_num in range(total_pages):
        ...
        for repl in replacements:
            ...
            if should_skip(text) or repl.get("is_superscript"):
                ...
                # Sentence - check if needs translation
                if has_french(text):
                    # Sentence with French - needs agent translation
                    sentences_to_translate[text] = ""  # Will be filled by agent
                    repl["translated"] = text  # Use original as key for lookup later
                    repl["needs_translation"] = True
                    repl["page"] = page_num
                    all_text_elements.append(repl)
                    print(f"  [SENTENCE] {text[:60]}...")
                else:
                    # English sentence - keep as is
                    repl["translated"] = text
                    repl["page"] = page_num
                    all_text_elements.append(repl)
            else:
                # Short text (1-6 words) - always try dictionary
    ...
    if sentences_to_translate:
        ...
        if not os.path.exists(sentences_cache):
            ...
            # Save for agent translation - text as keys
            to_translate_file = sentences_cache.replace('_sentences.json', '_to_translate.json')
            with open(to_translate_file, 'w', encoding='utf-8') as f:
                json.dump(sentences_to_translate, f, ensure_ascii=False, indent=2)

            print(f"Saved to: {to_translate_file}")
            print("\n**ACTION REQUIRED:**")
            print(f"1. Translate {to_translate_file} with Task agent")
            print(f"   Format: {{\"french text\": \"english translation\"}}")
    ...
    # Replace original text with actual translations
    for elem in all_text_elements:
        if elem.get("needs_translation"):
            original_text = elem["original"]
            elem["translated"] = translated_sentences.get(original_text, original_text)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python method9_translate.py <input_pdf> <output_pdf>")
        sys.exit(1)
